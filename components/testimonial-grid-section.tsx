...
const TestimonialCard = ({ quote, name, company, avatar, type }) => {
  ...
  let cardClasses = `flex flex-col justify-between items-start overflow-hidden rounded-[10px] shadow-[0px_2px_4px_rgba(0,0,0,0.08)] relative ${padding} transition-all duration-500 hover:scale-105 hover:shadow-2xl group cursor-pointer`
  let quoteClasses = ""
  let nameClasses = ""
  let companyClasses = ""
  ...
  return (
    <div className={`${cardClasses} ${cardWidth} ${cardHeight}`}>
      {backgroundElements}
      <div
        className={`relative z-10 font-normal break-words transition-all duration-300 group-hover:translate-y-[-2px] ${quoteClasses}`}
      >
        {quote}
      </div>
      <div className="relative z-10 flex justify-start items-center gap-3">
        ...
        <div className="flex flex-col justify-start items-start gap-0.5">
          <div className={`transition-all duration-300 group-hover:translate-y-[-2px] ${nameClasses}`}>{name}</div>
...
export function TestimonialGridSection() {
  return (
    <section className="w-full px-5 overflow-hidden flex flex-col justify-start py-6 md:py-8 lg:py-14">
      ...
      <div className="w-full pt-0.5 pb-4 md:pb-6 lg:pb-10 flex flex-col md:flex-row justify-center items-stretch md:items-start gap-4 md:gap-4 lg:gap-6 max-w-[1100px] mx-auto">
